...
namespace Biomind.Data.Repositories.Base
{
    ...
    public class DeleteColumn
    {
        ...
        public DeleteColumn(string columnName, object data, MySqlDbType dbType)
        {
    ...
    public abstract class RepositoryDataAccess
    {
        ...
        protected MySqlCommand CreateCommand(MySqlConnection cn, string command)
        {
        ...
        protected bool ToDataTable(MySqlCommand cmd, MySqlConnection cn, out DataTable dt)
        {
            var isSuccess = true;
            if (HasInjection(cmd.CommandText))
                throw new Exception("Sql Injection attempted. Aborted");
        ...
        protected DataTable ToDataTable(MySqlCommand cmd)
        {
            try
            {
        ...
        where T : IBaseModel, new()
    {
            ...
            using (var cn = new MySqlConnection(ConnectionString))
            {
                ...
                using (var cmd = CreateCommand(cn, sql))
                {
                    for (var i = 0; i < Columns.Count; i++)
                    {
                        ...
                        var parameter = cmd.Parameters.Add(createColumn.ColumnName, createColumn.MySqlDbType);
        ...
        protected bool BulkInsert(DataTable dt, string tableName)
        {
            try
            {
                using (var cn = new MySqlConnection(ConnectionString))
                {
                    ...
                    var bulkCopy = new MySqlBulkCopy(cn, null)
                    {
            ...
            using (var cn = new MySqlConnection(ConnectionString))
            {
                using (var cmd = CreateCommand(cn, sql))
                {
                    for (var i = 0; i < Columns.Count; i++)
                    {
                        ...
                        var parameter = cmd.Parameters.Add(updateColumn.ColumnName, updateColumn.MySqlDbType);
        ...
        protected bool BaseDelete(DeleteColumn deleteColumn, out IEnumerable<T> items)
        {
            ...
            using (var cn = new MySqlConnection(ConnectionString))
            {
                ...
                using (var cmd = CreateCommand(cn, sql))
                {
                    var parameter = cmd.Parameters.Add(deleteColumn.ColumnName, deleteColumn.MySqlDbType);
        ...
        protected bool NoneQuery(MySqlConnection cn, MySqlCommand cmd)
        {
            if (HasInjection(cmd.CommandText))
                throw new Exception("Sql Injection attempted. Aborted");
