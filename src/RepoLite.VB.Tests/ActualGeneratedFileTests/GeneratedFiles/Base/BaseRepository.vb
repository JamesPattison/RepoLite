            ...
            While True
                                ...
                                Return _
                                    If _
                                        (boolComparison,
                                         Clause.Make(
                                             If _
                                                        (Not String.IsNullOrEmpty([alias]), $"[{[alias]}].[{colName}]",
                                                         $"[{colName}]")),
                                         Clause.Make(
                                             Clause.Make(
                                                 If _
                                                            (Not String.IsNullOrEmpty([alias]),
                ...
                ElseIf TypeOf expression Is MethodCallExpression Then
                    ...
                    If methodCall.Method = GetType(String).GetMethod("Contains", {GetType(String)}) Then
                        Return _
                            Clause.Make(Parse(methodCall.Object, xRef), "LIKE",
                    ...
                    if (methodCall.Method.Name = "CompareString")
                        If methodCall.Arguments.Count >= 2 Then
                            If TypeOf methodCall.Arguments(2) is ConstantExpression
                                ...
                                Return _
                                    Clause.Make(Parse(methodCall.Arguments(0), xRef),
...
#region "Combined Repository"
    ...
    Public MustInherit Class CombinedRepositoryBase
        ...
        Private Function GenerateQuery() As String
            ...
            Dim prop = JoinInfo.InitialType.GetProperty("Columns", BindingFlags.NonPublic Or BindingFlags.[Static])
            ...
            For Each join In JoinInfo.Joins
                ...
                prop = join.ModelType.GetProperty("Columns", BindingFlags.NonPublic Or BindingFlags.[Static])
                If prop Is Nothing Then _
                    Throw _
                        New Exception(
